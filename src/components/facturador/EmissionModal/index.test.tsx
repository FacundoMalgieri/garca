...
describe("EmissionModal", () => {
  ...
  it("genera preview con creds encriptadas + token + companyIndex", () => {
    ...
    const [plantillaArg, creds] = startPreview.mock.calls[0];
    expect(plantillaArg).toBe(PLANTILLA);
  ...
  it("confirmar llama confirm con la plantilla y creds", () => {
    ...
    expect(confirm).toHaveBeenCalledWith(PLANTILLA, expect.objectContaining({ turnstileToken: "TS-TOKEN", companyIndex: 2 }));
  });
