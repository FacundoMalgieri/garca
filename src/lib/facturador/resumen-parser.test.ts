...
describe("parseResumen", () => {
  it("extrae importe total, receptor y líneas del HTML real de RCEL", () => {
    ...
    expect(preview.lineas[0].cantidad).toBe(1);
    expect(preview.lineas[0].precioUnitario).toBe(1000);
    expect(preview.lineas[0].subtotal).toBe(1000);
    expect(preview.puntoVenta).toBe("3");
  });

  it("NO confunde la razón social del emisor con la del receptor", () => {
    const preview = parseResumen(html, { puntoVenta: "3", tipoComprobante: 11 });
    expect(preview.razonSocialReceptor).not.toContain("MALGIERI");
  });
});
